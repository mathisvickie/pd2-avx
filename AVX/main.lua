...
local DeactivateUnitBrain = function()
	...
	for u_key, u_data in pairs(managers.enemy:all_enemies()) do u_data.unit:brain():set_active(false) end
	for u_key, u_data in pairs(managers.enemy:all_civilians()) do u_data.unit:brain():set_active(false) end
end

local ReactivateUnitBrain = function()
	
	for u_key, u_data in pairs(managers.enemy:all_enemies()) do u_data.unit:brain():set_active(true) end
	for u_key, u_data in pairs(managers.enemy:all_civilians()) do u_data.unit:brain():set_active(true) end
end

local DeleteInvisibleBorders = function()
	...
	if(net_session) then
		...
		for _, unit in pairs(World:find_units_quick("all", 1)) do
			...
			if CollisionData[unit:name():key()] then
				...
				net_session.send_to_peers(net_session, 'remove_unit', unit)
				unit:set_slot(0)
			end
		end  
	end
...
local FixRestartButton = function()
	
	MenuCallbackHandler.singleplayer_restart = function() return true end
...
if LuaNetworking:IsHost() then
	...
	for i = 1, #Vehicles, 2 do
		if i + 1 == #Vehicles then
			table.insert(Options, { text = Vehicles[i], callback = StartHelicopterMOD, data = Vehicles[i+1] })
		else
			table.insert(Options, { text = Vehicles[i], callback = SpawnVehicle, data = Vehicles[i+1] })
		end
	end
	--table.insert(Options, { text = "Deactivate unit brain", callback = DeactivateUnitBrain })
	--table.insert(Options, { text = "Reactivate unit brain", callback = ReactivateUnitBrain })
	table.insert(Options, { text = "Delete invisible borders", callback = DeleteInvisibleBorders })
	--table.insert(Options, { text = "Fix restart button", callback = FixRestartButton })
	...
	if managers.hud then managers.hud:show_hint({ text = "Vehicles can be spawned only by a host." }) end
end
